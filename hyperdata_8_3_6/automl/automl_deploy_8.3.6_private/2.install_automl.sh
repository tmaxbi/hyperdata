...
# 4. install automl
helm install -n ${NAMESPACE} hyperdata-ai --set loadbalancerAddr=${NGINX_IP}:${NGINX_PORT} --set enableHttps=${ENABLE_HTTPS} --set imageSecret=${IMG_SECRET} --set image.scheduler=${SCHEDULER_IMG} --set image.automl=${AUTOML_IMG} --set image.doloader=${DOLOADER_IMG} --set image.fe=${FE_IMG} --set image.xgb=${XGB_IMG} --set image.rf=${RF_IMG} --set image.timeseries=${TIMESERIES_IMG} --set image.downloader=${DOWNLOADER_IMG} --set image.woori=${WOORI_IMG} --set image.resultuploader=${RESULTUPLOADER_IMG} --set image.domainserving=${DOMAIN_SERVING_IMG} --set image.wooriserving=${WOORI_SERVING_IMG} --set kubeflowAddr=${KFSERVING_ADDRESS} ${parent_path}/automl
