...
while : ; do
  ...
  sed -e 's|@NAMESPACE|'"$NAMESPACE"'|g; s|@TARGET_SVC_NAME|'"$TARGET_SVC_NAME"'|g; s|@PROXY_TIMEOUT|'"$PROXY_TIMEOUT"'|g; s|@TARGET_VIRTUAL_SVC_NAME|'"$TARGET_VIRTUAL_SVC_NAME"'|g' ingress-hd.yaml | kubectl apply -f -
  [[ $? -ne 0 ]] && RETRY=0
